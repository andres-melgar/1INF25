...
ostream & operator<<(ostream &out, GestorCanales &gestor_canales) {
    Canal vista[MAX_CANALES];
    for (int i = 0; i < MAX_CANALES; i++) {
        vista[i] = gestor_canales.vector_canales[i];
    }
    static bool de_menor_a_mayor = true;
    if (de_menor_a_mayor)
        qsort(vista, MAX_CANALES, sizeof(vista[0]), ascendente);
    else
        qsort(vista, MAX_CANALES, sizeof(vista[0]), descendente);
    de_menor_a_mayor = not de_menor_a_mayor;

    for (int i=0; i<10; i++) {
        out << i+1 << " -> " << vista[i] << endl;
    }
    return out;
}