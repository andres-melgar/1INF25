...
void operator>>(istream &archivo_de_platos, ConjuntoDePlatos &conjuntoDePlatos) {
    Plato buffer_platos[200];

    int i = 0;
    while (archivo_de_platos >> buffer_platos[i]) {
        i++;
    }
    conjuntoDePlatos.cantidad = i;
    conjuntoDePlatos.conjunto_de_datos = new Plato[conjuntoDePlatos.cantidad];

    for (i = 0; i < conjuntoDePlatos.cantidad; i++)
        conjuntoDePlatos.conjunto_de_datos[i] = buffer_platos[i];
}
